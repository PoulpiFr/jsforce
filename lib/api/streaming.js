 ...
 */
Topic.prototype.subscribe = function(opts, listener) {
  ...
  opts = opts || {};
  return this._streaming.subscribe(this.name, opts, listener);
};
...
/** @private **/
Streaming.prototype._createClient = function(replay, opts) {
  var endpointUrl = [
  ...
  if(opts.replayId) console.log('Starting at replayId = ' + opts.replayId);
    ...
    replay = {
      ...
      if (message.channel === '/meta/handshake') {
        if (message.ext && message.ext['replay'] == true) {
          ...
          console.log('The extension is enabled. Oh yeah.');
        }
      }
      callback(message);
      },
      ...
      console.log('outgoing: ' + JSON.stringify(message, null, 4));
      callback(message);
      }
  };

  fayeClient.addExtension(replay);

  return fayeClient;
};

/** @private **/
Streaming.prototype._getFayeClient = function(channelName, opts) {
  ...
  if (!this._fayeClients || !this._fayeClients[clientType]) {
    ...
    this._fayeClients[clientType] = this._createClient(isGeneric, opts);
 ...
 * @returns {Subscription} - Faye subscription object
 */
Streaming.prototype.subscribe = function(name, opts, listener) {
  var channelName = name.indexOf('/') === 0 ? name : '/topic/' + name;
  var fayeClient = this._getFayeClient(channelName, opts);
